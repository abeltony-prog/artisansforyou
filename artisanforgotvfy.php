          ...
          <div class="col-md-6 py-5">
            <?php
              if (isset($_POST['reset'])) {
                $vkey = $_POST['code'];
                if (DB::query('SELECT password FROM artisans WHERE password=:password', array(':password'=>$_GET['password']))[0]['password']) {
                  $code = DB::query('SELECT * FROM artisans WHERE password=:password', array(':password'=>$_GET['password']));
                  foreach ($code as $key) {
                    if ($vkey == $key['vkey']) {
                      header('Location:artisanChangerpassword.php?artisan='.$key['id']);
                    }else {
                      ?>
