          ...
          <div class="col-md-6 py-5">
            <?php
              if (isset($_POST['reset'])) {
                $artisanemail = $_POST['email'];
                $email = DB::query('SELECT * FROM artisans WHERE email=:email', array(':email'=>$artisanemail));
                if (DB::query('SELECT * FROM artisans WHERE email=:email', array(':email'=>$artisanemail))) {
                  foreach ($email as $key) {
                    $token = $key['vkey']."786";
                    $token= str_shuffle($token);
                    $token = substr($token , 0,5);
                    DB::query('UPDATE artisans SET vkey=:token WHERE email=:email', array(':email'=>$key['email'],':token'=>$token));
                    header('Location:artisanforgotvfy.php?password='.$key['password']);
                  }
                }else {
                  ?>
            ...
            <form action="" method="post">
              ...
              <p><a href="ArtisanLogin.php">Remember password?</a></p>
